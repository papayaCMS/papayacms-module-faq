...
* @version $Id: output_faq.php 39730 2014-04-07 21:05:30Z weinert $
*/
          ...
          if (isset($entry['faqgroup_id']) && $entry['faqgroup_id'] == $i) {
            ...
            if ($isSelectedEntry) {
              if ($data['post_comments'] != -1) {
                if ($this->papaya()->surfer->hasPerm($data['post_comments'])) {
  ...
  */
  function initializeOutputPermForm() {
    if (!is_object($this->outputDialog)) {
      ...
      if ($this->papaya()->surfer->isValid) {
        $data['note_username'] =
          $this->papaya()->surfer->surfer['surfer_givenname'].' '.
          $this->papaya()->surfer->surfer['surfer_surname'];
        $usernameInput = FALSE;
      } else {
        $usernameInput = TRUE;
      }
  ...
  */
  function addCommentOutput($parent, $text, $username, $data) {
    if ($this->entryExists($parent)) {
      if ($data['post_comments'] != -1 &&
          !(
            $this->papaya()->surfer->isValid &&
            $this->papaya()->surfer->hasPerm($data['post_comments'])
           )
          ) {
        return FALSE;
      } else {
