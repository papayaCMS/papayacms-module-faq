...
* @subpackage Free-FAQ
* @version $Id: content_faq.php 39504 2014-03-04 11:15:22Z weinert $
*/
...
class content_faq extends base_content {
  ...
  */
  function getFaqCombo($name, $field, $data) {
    ...
    if (isset($faqObject->faqs) && is_array($faqObject->faqs)) {
      foreach ($faqObject->faqs as $faq) {
  ...
  */
  function getFaqGroupCombo($name, $field, $data) {
    ...
    if (isset($this->data['faq']) && (int)$this->data['faq'] > 0) {
      $faqId = $this->data['faq'];
      $faqObject = new base_faq();
      $faqObject->loadFaqGroups($faqId);
      if (isset($faqObject->faqGroups) && is_array($faqObject->faqGroups)) {
        foreach ($faqObject->faqGroups as $faqGroup) {
