...
* @subpackage Free-FAQ
* @version $Id: admin_faq.php 39818 2014-05-13 13:15:13Z weinert $
*/
  ...
  */
  function getXML() {
    if (is_object($this->layout)) {
      $this->layout->parameters()->set('COLUMNWIDTH_LEFT', '300px');
      $this->getXMLButtons();
      $this->getXMLTreeList();
      ...
      if ($dialog->execute()) {
        $this->papaya()->messages->dispatch(
          new PapayaMessageDisplayTranslated(
            PapayaMessage::SEVERITY_INFO, 'Faq group saved.'
          )
        );
  ...
  function addComment($parent) {
    if ($this->entryExists($parent)) {
      $data = array(
        ...
        'note_username' => $this->papaya()->administrationUser->user['username'],
        'note_created' => time()
      );
